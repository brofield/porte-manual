...
<xsl:template match="/">
	<html>
		...
		<body onload="boVis(5);printAndCloseZ();barBlock();">
			<table width="100%" cellspacing="0" cellpadding="2" style="display:none" id="print">
				<tr> 
					<td align="left" width="20%" id="print3"> 
						<input title="チェックの入った項目を印刷します" type="submit" name="submit3" value="印刷確認" onClick="printSelect()" />
					</td>
					<td align="left" width="20%" style="display:none" id="print4"> 
						<input type="submit" name="submit3" value="印刷" onClick="printCheck()" />
					</td>
					...
					<td align="right" width="20%"> 
						<input type="submit" name="submit22" value="閉じる" onClick="closeWindow()" />
					</td>
				</tr>
			</table>
	<form name="divValue">
		<xsl:element name="input">
			...
			<xsl:attribute name="value"><xsl:value-of select="$printNumber"/></xsl:attribute>
		</xsl:element>
<!--
...
<xsl:template match="SECTION">
	<xsl:variable name="pringNumber2">
		<xsl:number format="1" level="any" count="SECTION"/>
	</xsl:variable>
	<xsl:element name="div">
		<xsl:attribute name="id">bm<xsl:value-of select="$pringNumber2 - 1" /></xsl:attribute>
		<table width="100%" cellspacing="0" cellpadding="0">
			<xsl:attribute name="id">bar<xsl:value-of select="$pringNumber2 - 1" /></xsl:attribute>
			<xsl:attribute name="style">display:none</xsl:attribute>
			<tr> 
			<xsl:choose>
			<xsl:when test="child::PARA/@INFOTYPE[. = 'SYSTEM']">
				<td align="center" class="blue">
					<table border="0" cellspacing="0" cellpadding="0" width="100%">
						<tr>
							<td width="5%" class="blue">
								<xsl:element name="input">
									<xsl:attribute name="type">checkbox</xsl:attribute>
									<xsl:attribute name="name">check<xsl:value-of select="$pringNumber2 - 1" /></xsl:attribute>
									<xsl:attribute name="value">checkbox</xsl:attribute>
									<xsl:attribute name="style">display:none</xsl:attribute>
								</xsl:element>
							</td>
							<td width="80%" class="blue" align="center"><xsl:value-of select="SCTITLE" /></td>
							<td align="right" class="blue" width="15%"> </td>
						</tr>
					</table>
				</td>
			</xsl:when>
			<xsl:when test="child::PARA/@INFOTYPE[. = 'PARTS']">
				<td align="center" class="gray">
					<table border="0" cellspacing="0" cellpadding="0" width="100%">
						<tr>
							...
							<td width="80%" class="gray" align="center"><xsl:value-of select="SCTITLE" /></td>
		...
		<xsl:element name="div">
			<xsl:attribute name="class">frame</xsl:attribute>
				<xsl:element name="span">
					<xsl:attribute name="onClick">javascript:treeMenu('tm<xsl:value-of select="@SCID"/>');chgImg('im<xsl:value-of select="@SCID"/>');</xsl:attribute>
					...
					<xsl:attribute name="style">cursor:hand;color:#1B6384;text-decoration = 'underline'</xsl:attribute>
						<xsl:element name="img">
							...
							<xsl:attribute name="id">im<xsl:value-of select="@SCID"/></xsl:attribute>
				...
				<br/>
			<xsl:element name="a">
				<xsl:attribute name="name"><xsl:value-of select="$pringNumber2 - 1" /></xsl:attribute>
			</xsl:element>
			<xsl:element name="div">
				<xsl:attribute name="style">display:none</xsl:attribute>
				<xsl:attribute name="id">tm<xsl:value-of select="@SCID"/></xsl:attribute>
				<xsl:apply-templates />
			</xsl:element>
		</xsl:element>
		</xsl:if>
...
<xsl:template match="TITLE">
	...
	<xsl:when test="parent::PARA">
		<br />
		<div class="blue2"><xsl:apply-templates/></div>
<!--
		<br />
		<table width="100%" cellspacing="0" cellpadding="0">
			<tr>
				...
				<xsl:when test="parent::PARA/@INFOTYPE='SYSTEM'">
					...
					<td align="right" class="blue" width="15%"><xsl:call-template name="memo" /></td>
				</xsl:when>
				<xsl:when test="parent::PARA/@INFOTYPE='SYSTEM'">
					<td width="5%" class="gray"></td>
					<td width="80%" class="gray" align="center"><xsl:apply-templates/></td>
					<td align="right" class="gray" width="15%"><xsl:call-template name="memo" /></td>
				</xsl:when>
				<xsl:otherwise>
...
<xsl:template match="HINTREF">
	...
	<xsl:choose>
	<xsl:when test="ancestor::TABLE">
		<xsl:for-each select="ancestor::TABLE/following::HINT[1]/LIST1/ITEM">
